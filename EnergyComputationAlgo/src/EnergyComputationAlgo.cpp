...
typedef Triplet<complex<double> > ComplexTriplet;
	...
	complex<double> eta_concrete = complex<double>(2.55, -0.01);
//	complex<double> eta_concrete = complex<double>(1.326, -0.043);
	complex<double> eta_wood = complex<double>(1.27, -0.042);
	// Refraction index for air 1.0
	complex<double> eta_air = complex<double>(1.0, 0.0);
	...
	vector<vector<double> > EnergyComputationAlgo::computeSignalStrengths(vector<vector<double> > imageMatrix, int routerXpos, int routerYpos, int materialType, double freq) {
		...
		complex<double> airConstant = pow(k / eta_air, 2);
		complex<double> wallMaterialConstant;

		switch(materialType) {
		case 1: wallMaterialConstant = pow(k / eta_concrete, 2);
		break;
		case 2: wallMaterialConstant = pow(k / eta_wood, 2);
		break;
		default: wallMaterialConstant = pow(k / eta_concrete, 2);
		break;
		}
		...
		for (int x = 1; x <= dimx; x++) {
			for (int y = 1; y <= dimy; y++) {
				...
				vVal = plan(x-1, y-1) - (2 * pow(delta, -2));
